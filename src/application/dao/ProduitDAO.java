...
public class ProduitDAO {
    ...
    public ArrayList<Produit> getAll() throws SQLException {
        ...
        try (Statement stmt = connection.createStatement(); ResultSet rs = stmt.executeQuery(query)) {
            while (rs.next()) {
                list.add(new Produit(rs.getInt("id"), rs.getString("nom"), rs.getDouble("prixVente"), rs.getInt("seuilMinimal")));
            }
        }
        return list;
    }
    ...
    public void update(Produit p) throws SQLException {
        String query = "UPDATE PRODUIT SET nom=?, prixVente=?, seuilMinimal=? WHERE id=?";
        try (PreparedStatement stmt = connection.prepareStatement(query)) {
            stmt.setString(1, p.getNom());
            stmt.setDouble(2, p.getPrixVente());
            stmt.setInt(3, p.getSeuilMinimal());
